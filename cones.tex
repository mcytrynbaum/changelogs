...
\begin{enumerate}
\item If there exists a $k$ such that $v_i, v_j \in c(v_k)$, then $\fix(\pre_i) = \fix(\pre_j)$. 
\item If $v_i = p(v_j)$, then $\fix(\pre_j) \sub \fix(\pre_i)$. 
\end{itemize} 

Thus, siblings share fixed points, and the fixed points of a parent are contained in the fixed points of each of its children.
...
For each $\preo_i \in \acto$, form a digraph $G = (V,E)$ by letting $(i,j) \in E$ if and only if there exists $\pre_j \in Q$ such that (i) $\pref_i \peq \pre_j$, (ii) $\preo_i \not \peq \pre_j$, and (iii) $r(j) = k$.  
Next, form the strong components\footnote{Tarjan's algorithm for computing $\stongc$ runs in time $\mathcal{O}(|V| + |E|)$ for an arbitrary digraph $G$.}
of the digraph $G$, denoted by $\strongc = \{W_j\}_{j\in \mathcal{J}}$, where $\mathcal{J}$ is some index set. 
For $j \in \mathcal{J}$, pick $v \in W_j$ and compute the reachable set of $v$, denoted by $\reach_j$, for instance by breadth-first search. 
Since strong components are strongly connected, this definition is independent of the choice of $v$. 
Note that, by the definitions above, we have $\fix(\pre_i) \sub$ 
