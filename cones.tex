...
\item Adding a vertex $w$ and directed edges $(w,w')$, where $w' \in \roott(\forest_0)$.
\end{enumerate}
Then $\forest$ is also a rooted forest.
\end{lemma}

\prf
Note that a graph $G = (V,E)$ is a tree if and only if $G$ is a connected graph such that $|E| = |V| - 1$\footnote{Gross and Yellen xxxx}. 
...
Now we come to the proof of the key lemma for correctness. 
