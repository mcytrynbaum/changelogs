...
\subsection{Model and Notation} 
In this section, we restrict our attention to two-sided bilateral matching with contracts, and we may think of the contract language as $X = D\times H \times E$, where $E$ is a finite set of contract terms; for instance, $E$ could be a set of wages.
...
\subsubsection{Lattice Definition and Fixed Point Guarantee}
...
For any $(Z,W) \in \fix(\Phi)$, we have $Z\cap W \in \stable$.
...
Specifically, for a fixed element $(X_i, Y_i) \suq (\botx, \boty)$, where $i$ is some index, we let $\lattice_i \equiv \{(Z,W): (Z,W) \peq (X_i, Y_i) \}$ and note that this is a complete sublattice.
Define $\Phi_i: \lattice_i \to \lattice_i$ by setting $\Phi_i(Z,W) = (\Phi^D_i(W), \Phi^H_i(Z))$, where $\Phi^D_i(W) = \{z \in X_i \setminus \botx: z \in C_H(\{z\} \cup Z)\}) \cup \botx$ and $\Phi^H_i(Z) = \{z \in \topy \setminus Y_i: z \in C_D( \{z \} \cup W \}) \cup Y_i$.

The following result shows that this family of suboperators satisfies the conditions required in our discussion of information sharing on general complete lattices.

\begin{lemma} \label{lemma:phigar}
Let $\lattice_i$ and $\Phi_i$ as defined above for some $(X_i, Y_i) \suq (\botx, \boty)$. 
Then $\Phi_i$ is isotone on the sublattice $\lattice_i$.
\end{lemma}
\prf 
\eprf
