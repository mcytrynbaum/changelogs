...
\REQUIRE $\acto$
    ...
    \FOR{$\pre \in \acto$}   %processing
        \IF{$\exists \pre' \in \acto$ s.t. $\pre \peq \pre'$}
            \STATE $\acta \leftarrow \acta \setminus \pre$
        \ENDIF
    \WHILE{$\acta \not = \emptyset$}
        \FOR{\pre \in \acta} 
            \STATE $\pre' \leftarrow T\pre$
            \IF $\pre' = \pre$
                \STATE Remove $\pre$ from $\acto, \acta$
