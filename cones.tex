    ...
    \WHILE{$\acta \not = \emptyset$}
        ...
        \STATE $\pre \leftarrow \pre_i$
            \WHILE{$\stp = $ \FALSE} 
                ...
                \IF{$\pre' = \pre$}
                    \STATE Remove $\pre_i$ from $\acto$ and $\pre$ from $\acta$ 
                    \STATE Set $\pref \leftarrow \pre$; Add $v(\pref)$ to $F$ with $c(v(\pref)) = v(\pre_i)$
                    ...
                    \ELSE[Stopping Condition 2]
                        ...
                        \ENDFOR
                    \ENDIF
                \ELSIF[Stopping Condition 3]{$\pre' \peq \bopre$}  
                    \STATE Remove $\pre_i$ from $\acto$ and $\pre$ from $\acta$ 
...
need to actually add \emph{all} final values to reach
