...
Starting with $\toppre_i$, the unanimously most preferred preallocation in the subproblem $\lag \toppre_i \rag$, each iteration of $S_i$ gives a ``cone guarantee'' on $\fix(T)$ of the form $\fix(\pre - e_i) \peq S_i^k (\toppre - e_i)$, where we have applied lemma xxxx and the Tarski argument from above. 

Pursuing this strategy for $i = 1 \hdots m$, we can bound the complete set of fixed points $\fix(T)$ in a union of cones corresponding to each subproblem. 
Continuing recursively in this way, and generating $m$ new subproblems whenever a sequence of the form $S_i^k \toppre_i $ stops at a fixed point of $S_i^k$, Echenique and Yenmez. 

This strategy is actually similar to greedy search.
In particular, the number of subproblems generated at the kth recursive level scales exponentially in $k$ as $|A|^k$.

As we will show, the strategy described above can be significantly improved by capitalizing on the geometric structure of the lattice of preallocations to allow subproblems to share information with each other. 
With this modification, evaluation of the full Tarski sequence $S_i^k \toppre_i$ is often unnecessary. 
In fact, it is often the case that subproblems can be stopped or removed entirly after a few iterations. 
We illustrate this with a couple of simple examples.  
\subsubsection{Example 1 - Colliding Subproblem Cones}

\subsubsection{Example 2 - Equivalent Problems}
