...
Our proof of correctness will also make use of the following lemma, which shows that we can track problem relations with the graph $F$.
...
\item If $w_j = r(w_i)$ and $k \in \coll(i)$, then $\fix(\pre_j) \sub \fix(\pre_k)$ \label{convlemma:coll}
\end{enumerate}
\end{lemma}

First, a definition. 
For a forest (a collection of rooted trees) define the \emph{root set} $\root(\forest) = \{r(w): w\in F\}$. 
That is, $\roott(\forest)$ consists of all roots of trees in the forest.

Next, we have the following auxillary lemma

\begin{lemma}
Let $\forest_0$ be a rooted forest. 
Suppose that $\forest$ is obtained from $\forest_0$ in one of the following ways 
\begin{enumerate}
\item Adding an isolated vertex to $\forest_0$ 
\item Adding a vertex $w$ and edges $(w,w')$, where $w' \in \roott(\forest_0)$.
\end{enumerate}
Then $\forest$ is also a rooted forest.
\end{lemma}

\prf
We will make use of the following result: suppose that $G = (V,E)$ is a connected graph with $|V| = n$ and $|E| = n-1$, then $G$ is a tree\footnote{Gross and Yellen}. 
...
Each $t_i$ is a tree by assumption. 
$\{w\}$ is also trivially a rooted tree (with root $w$) by the graph-theoretic result above. 

For case 2, let $w$ be the added vertex and $J$ the subset of tree indices such that edge $(w,\roott(t_i))$ is added to $\forest_0$.
Let $i \in J$. 
...
It suffices to show that $t^*$ is a tree. 
By construction, $|V(t^*)| = 1 + \sum_{i \in J} |V(t_i)|$ and $|E(t^*)| = |J| + \sum_{i \in J} |E(t_i)| = |J| + \sum_{i \in J} (|V(t_i)| - 1) = \sum_{i \in J} |V(t_i)|$. 
Then $|V(t^*)| = |E(t^*)| + 1$, and, by the criterion above, $t^*$ is a tree.

handle the rooted part 
...
A straightforward induction on distinct states $(\act,\forest)_k$ of the pair $(\act, \forest)$ shows that condition~\ref{def:forest1} of the definition is always satisfied. 
By inspection, any line in the algorithm (lines~\ref{alg1:addq} of subroutine~\ref{infac} and \ref{alg2:addq} of subroutine~\ref{infcomb}) where we add to $\act$ is paired with a preceding line, where we add all new points $\pre \in Q$ to $\forest$ as $w(\pre)$, so this condition clearly holds. 

For the other statements, we work by induction on the sequence $\pair_k$ of distinct states of the pair $\pair$. 
We argue by induction that items (1) and (2) above hold. 
Our proof will also show that every modification of $\forest$ takes one of the following forms 
\begin{enumerate}
\item Adding an isolated vertex to $\forest$ 
\item Adding a vertex $w$ and edges $(w,w')$, where $w' \in \roott(\forest)$.
\end{enumerate}

By lemma xxxx this will show by induction that $\forest$ is a rooted forest at all times, as claimed. 
...
At this line, we set $\pre_i^f = \pre' = (T_i^2)^{\ell}$ for some $\ell \geq 0$.
...
Note that for any $\pre_i \in \acto$, $w_i$ is a root in $\forest$, 
Suppose that $\pre_i^f = \pre_k$
...
why don't we define a root set 
...
show lemma about tree construction 
