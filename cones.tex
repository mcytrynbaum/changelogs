...
This extension informs our algorithm for finding all stable allocations. 
We also show briefly how these ideas can be used to extend the algorithm for finding all pure strategy Nash equilibria in games with strategic complementarities (GSC) pioneered in Echenique (2003).
...
Specifically, suppose that we can show $\fix(\oper)$ satisfies $\fix(\oper) \cap \lattice_i \sub \fix(\oper_i)$.
...
Forming the Tarski sequence for $\oper_i$ on the sublattice $\lattice_i$, we find $y_i = \max \fix(\oper_i)$, obtaining a guarantee of the form $\point_i \sub \fix(\oper) \cap \lattice_i \sub \fix(\oper_i) \peq y_i$.
...
To fully extend the algorithm, we discuss the intialization of new subproblems when, for instance, a Tarski sequence for the subproblem on $\lattice_i$ hits a fixed point of $\oper_i$.
...
For more general problems, we must work with whatever regularity is offered by the lattice.
