...
\REQUIRE $\acto$, $\coll$
    ...
    \STATE Remove all $\{v_k\}\in \strongc$ with $\pre_k \not \in \acto$, leaving $\{S_j\}_{j \in \mathcal{J}}$ \label{alg2:correct_strong}
...
Note that if for any $k\geq 0$ we have $\acta^{k+1} = \actak \setminus \{\pre\}$ for some $\pre \in \prealloc$, then $d(\acta^{k+1}) \leq d(\actak)$.
By inspection, all changes to $\acta$ except for the change on line~\ref{alg1:addq} are of this form.
For line~\ref{alg1:addq}, note that it is always the case that $\|\pre\|_1 > \max_i \|\pre - e_i\|$, so, in fact, if $\actak \to \acta^{k+1}$ on line~\ref{alg1:addq} then $d(\acta^{k+1}) \leq d(\actak)$. 
This shows that $d(\actak)$ is a monotonically decreasing sequence. 
Since $d(\actak)$ takes at most finitely values, this sequence of real numbers converges in finite time.
...
Since $d(\actak)$ converges in finite time, the for loop at line~\ref{alg1:for2} is executed at most finitely many times. 
...
We will show the sequence of positive real numbers $D(\actok)$ is monotone decreasing.
Consider a single execution of InformationCombine with input $\acto$.
Let $\strongcomp \in \strongc$ be a strong component after removing irrelevant singletons in line~\ref{alg2:correct_strong}. 
By construction, the out-degree of $v_i \in V$ is $0$ if $\pre_i \not \in \acto$
By definition of a strong component, then, each vertex $v_i$ \emph{not} corresponding to a preallocation $\pre_i \in \acto$ generates a singleton strong component $\{v_i\}$ and is removed at line~\ref{algo2:correct_strong}. 
Then $\{v_i: v_i \in \strongcomp_j, j \in \mathcal{J}\} \sub \{v_i: \pre_i \sub \acto\}$. 
Therefore, by disjointness of strong components $\{\strongcomp_j\}_{j \in \mathcal{J}}$, there exists a surjection $f: \acto \to \actt$ of the form 
