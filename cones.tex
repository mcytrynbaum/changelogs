...
In this section, we give an algorithm that finds \emph{all} strict core allocations in the multilateral matching with contracts model considered above.
...
Let $\fix(\pre) \equiv \{\pre' \in \fix(T): \: \pre' \pe \pre\}$. 

By Tarski's theorem, the sequence $(T^2)^k \pre^*$ converges to a preallocation $\toppre$, fixed by $T^2$, which is the maximal point of the lattice $\fix(T^2)$.
If $T \toppre = \toppre$, then by lemma xxxx we know $\fix(T) = \{\toppre\}$.
Suppose then that this is not the case. 
Clearly $\fix(T) \sub \fix(T^2)$ for any operator, and, by iterating $T^2$, we have learned that $\pre \peq \toppre$ for all $\pre \in \fix(T^2)$, so $\pre \peq \toppre$ for all $\pre \in \fix(T)$ as well.

That is, iterating $T^2$ from the problem's maximal point $\pre^*$ gives us a guarantee that $\fix(T)$ is contained in the cone $\{\pre' \in \prealloc: \pre' \peq \toppre\} \equiv \{\pre' \peq \toppre\}.$ 
In what follows, we will often denote this type of cone relation by $\fix(T) \peq \toppre$. 

Since $T\toppre \not = \toppre$, apparently $\fix(T) \sub \bigcup_{i = 1}^m \{\pre' \peq \toppre - e_i\}$, where $e_i$ is the standard unit vector in $\mr^m$ \footnote{identifying $\prealloc$ with a grid in $\mr^m$ as above}. 
Consider a subproblem of the form $\lag \pre - e_i \rag$, and let $T_i$ be the corresponding operator for agents preferences truncated above $\pre - e_i$.
The key insight of Echenique (2003), generalized to the current setting, is that $\fix(T) \cap \{\pre' \leq \pre - e_i\} \sub \pre(T_i) \sub \fix(T_i^2)$, where the last subset relation holds for any operator.
Thus, information about fixed points of $T_i^2$ can be used to bound $\fix(T)$. 
...
Give example of crash (pic)
Give example of chaning (pic) 
Give algo and proofs 
