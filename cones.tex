...
We will show by induction that for all $\pre \in \acto_{\ell}$, if $\pre'' \in \fix(\pre)$, then either $\pre'' \in \fixtemp_{\ell}$ or there exists a guarantee $\pre' \in \acta_{\ell} \cup \actc_{\ell}$ with $\pre'' \peq \pre' \pre \pre$ (g) afforded by some $\pre' \in \acta_{\ell} \cup \actc_{\ell}$.
...
By the Tarski argument discussed earlier in the text, $\fix(\pre_i) \sub \fix(\pre')$.
Let $\preo \in \acto$ and $\pre \in \fix(\preo)$ and suppose that $\pre \not \in \fixtemp_{k + 1}$.
Then, in particular, $\pre \not = \pre'$, since we check if $\pre' \in \fix(T)$ when processing $\pre_i$.
So if $\pre \peq \pre'$, then $\pre \pe \pre'$ strictly.
But by lemma xxxx and because $\fix(T) \sub \fix(T^2)$, we actually have $\fix(\pre') = \emptyset$, so this is impossible.
By the inductive hypothesis, there must be $\pre'' \in \actak \cup \actck$ such that $\pre \peq \pre'' \peq \preo$ and $\pre'' \not = \pre'$.
...
Again, we have $\pre'$ the lowest element of the Tarski sequence for $\lag \pre_i \rag$ with $T_i^2 \pre' = \pre'$ by assumption, where, as above, $\fix(\pre_i) \sub \fix(\pre')$.
We then remove $\pre_i$ and add $\pre' - e_i$ for $1 \leq i \leq m$, so $\actakk = \actak \setminus \{\pre_i \} \cup \left \bigcup_{i \in I} \{\pre' - e_i \}$, where $I \sub \{1,\hdots,m\}$ are the indices such that $\pre' - e_i$ is non-redundant (as in line~\ref{alg1:addq}).
Let $\preo \in \acto$ and $\pre \in \fix(\preo)$ and suppose that $\pre \not \in \fixtemp_{k +1}$.
If $\pre \peq \pre_i \peq \preo$ as in (g) above, then, in particular, $\pre \peq \pre'$ by the Tarski argument.
...
If $i \in I$ then $\pre \peq \pre' - e_i \pe \pre' \peq \preo$, so $\pre' - e_i$ gives a new guarantee (g) on this fixed point.
No other points of $\actak$ are removed, so we are done.
Suppose, conversely, that $i \not \in I$; that is, the problem $\pre' - e_i$ is redundant.
Then $\pre - e_i = \pre'' \in \actok$.
By working with the guarantee (g) on $\pre'' \in \acto$ (rather than the guarantee on $\preo$), there is $\pre_g \in \actak \cup \actck$ such that $\pre \peq \pre_g \peq \pre'' = \pre' - e_i \pe \pre' \pre \pre_i$.
In particular, $\pre_g \not = \pre_i$, so it is not removed and $\pre_g \in \actkk \cup \actckk$.
