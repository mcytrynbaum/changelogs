...
\draw[fill] (3,5) circle [radius=0.05];
\draw[fill] (6,2) circle [radius=0.05];
\draw[->, ultra thick] (5,6) to [out=180, in=70] (3,5);
\draw[->, ultra thick] (6,5) to (6,2);
\draw[darkgray, ultra thick] (0,6) -- (5,6) -- (5,0);
\draw[darkgray, ultra thick] (0,5) -- (6,5) -- (6,0);
\draw[blue] (0,5) -- (3,5) -- (3,0);
\draw[blue] (0,2) -- (6,2) -- (6,0);
\draw[pattern = dots, pattern color = blue] (0,5) rectangle (3,0);
\draw[pattern = dots, pattern color = blue] (0,0) rectangle (6,2);
\draw[pattern = north west lines, pattern color = green] (0,0) rectangle (3,2);
\node[right] at (6,6) {(10,10)};
...
In particular, the algorithm finds all strict core outcomes $\strcore = \{Y: \pre_Y \in \fixfind \}$. 
\end{thm}
\prf
...
We then remove $\pre_i$ and add $\pre' - e_i$ for $1 \leq i \leq m$, so $\actatt = \actat \setminus \{\pre_i \} \cup \left (\bigcup_{i \in I} \{\pre' - e_i \} \right )$, where $I \sub \{1,\hdots,m\}$ are the indices such that $\pre' - e_i$ is non-redundant (as in line~\ref{alg1:addq}).
...
\emph{Remark}: Consider the worst case scenario where $\Phi_i: \lattice_i \to \lattice_i$ fixes at $\pairi = \max \lattice_i$ for every $(\botx, \boty) \peq \pairi \peq (\topx, \topy)$.
...
rewrite algo and proofs using new index
