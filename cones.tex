        ...
        \STATE $\pre \leftarrow \pre_i$
            \WHILE{$\pre_i \in \acta$} 
                ...
                \IF[Stopping conditions 1 and 3]{$\pre' \peq \bopre$ \OR $\pre' = \pre$}
                    ...
                    \ELSIF{$\pre' = \pre$}
                        \FOR{$i = 1$ to $m$}
                            \IF[Add non-redundant problems]{$\nexists \pre'' \in \acto$ with $\pre - e_i = \pre''$ } \label{alg1:redundant}
...
Before proving our key result, we show an auxillary lemma about the relationship between $\forest$ and the problem sets returned from each subroutine 

\begin{lemma} \label{lemma:roots}
Any distinct problem $\pre \in \prealloc$, is processed (the Tarski sequence for $\lag \pre \rag$ is evaluated) at most once in subroutine 1.
...
\emph{Case 6}: There is a transition $\pair_n \to \pair_{n + 1}$ in subroutine~\ref{infac} when we add collision index $j$ to $\coll_n(i)$ for some newly created problem $\pre_i \in \actt$ (line~\ref{alg2:coll2}).
...
\end{lemma}
\prf
For any set $E \sub \prealloc$, define $\disto(E) = \max_{\pre \in E} \|\pre\|_1$, where the set of preallocations $\prealloc$ is identified with a grid as above\footnote{For $\x \in \mr^m$, $\|x\|_1 = \sum_{i = 1}^m |x_i|$ is the standard 1-norm .}.
...
First, we show that, for any problem collection $\acto$, InformationAcquire (subroutine~\ref{infac}) terminates in finitely many iterations.
...
The algorithm returns $\acto' \equiv \actc$ which is an input for subroutine~\ref{infcomb}.
...
Therefore, $\pre' = \min_{\ell \in K_j} \pre_{\ell} \peq \pre_k \peq \pre_i^f \pe \pre_i$, where the second inequality follows by lemma~\ref{lemma:key}, which shows that $\forest$ is a problem forest, and because $w(\pre_i^f)$ is a descendant of $w(\pre_k)$.
...
FindStrictCore finds all fixed points $\fix(T)$. 
\end{thm}
\prf
...
Then $\fixfind = \fix(T)$. 
That is, by lemma~\ref{lemma:strictcore} above, the algorithm finds all strict core allocations.
\eprf

\emph{Discussion}: talk about main algorithmic advantages, how this is incommensurable with E and Y and depends on the structure of the lattice (but there may be a prob. type result to be had).

\subsubsection{Extension to True Core Allocations} It is tempting to attempt to extend the fixed point construction $\strcore = \fecon(T)$ and associated algorithm on the lattice $\prealloc$ to class of true core allocations $\core \supseteq \strcore$ in the general mathcing with contracts setting considered above. 
...
For instance, this is the case in the model of many-to-one matching with preferences over colleagues considered in the original paper by Echenique and Yenmez\footnote{something about other examples of this}.

At first glance, it appears that such a generalization will be forthcoming. 
As in the case of strict core, for $a \in A$ we define a set 
...
With these modifications, we get a similar fixed point characterization.
...
We have thus characterized the core allocations $\core$ as the fixed points of an operator defined on the preallocation lattice $\prealloc$.
...
\end{prop}
\prf 
Consider an embryonic trading network with two sellers and a buyer $A = \{s_1,s_2,b\}$. 
...
Since $\pre'(a) = \emptyset_a$ for all $a$, so that any trade is a strict improvement for each agent involved, so every agent can propose its most preferred network structure.
...
\begin{lemma}[Hatfield and Kominers, Lemma 1]
...
\subsubsection{Fixed Point Guarantees and Initializing Subproblems}
...
\emph{Initialization of Subproblems}: 
...
\subsection{Algorithm for Finding all Stable Matchings} \label{section:algostable}

In this section, we describe the full information sharing algorithm for finding all stable allocations in two-sided many-to-many matching with contracts, assuming substitutability of agents preferences.


\emph{Stopping Conditions}:
As above, we let $\act$ denote a collection of problems, which we think of previously solved problems that still contain information useful for finding $\fix(\Phi)$. 
During the algorithm, for a problem $\lag (X_i, Y_i) \rag $, we compute a Tarski sequence of the form $\Phi_i^k(X_i,Y_i)$, $k \geq 0$. 
...
For Condition~(\ref{cond:fix}), the Tarski sequence fixes, and we need to initialize new problems.
...
Iterate $\Phi^k(\emptyset,X)$ and $\Phi^k(X, \emptyset)$, which converge in finite iterations to $(\topx, \topy)$ and $(\botx,\boty)$, the largest and smallest elements of $\fix(\Phi)$, respectively.
Add each of these to the fixed point collection $\fixfind$.
For each $x \in \topx \setminus \botx$, add $(\topx \setminus \{x\}, \topy)$ to $\acto$ and $\act$ and add $w(\topx \setminus \{x\}, \topy)$ to $\forest$ as an isolated root
Similarly, for each $y \in \boty \setminus \topy$, add $(\topx, \topy \cup \{y\})$ to $\acto$ and $\act$ and add $w(\topx, \topy \cup \{y\})$ to $\forest$ as an isolated root.
...
For convenience, we reproduce the main algorithm here.

\begin{algorithm}[H] \label{mainalg2} 
\caption{FindStable} 
\begin{algorithmic}[1]
\REQUIRE $\acto$
    \STATE Initialize as above
    \STATE Set $\fixfind = \emptyset$
    \WHILE{$\acto \not = \emptyset$} \label{alg32:while1}
        \STATE $(\acto, \coll, \fixtemp) \leftarrow InformationAcquire(\acto)$ 
        \STATE $\fixfind = \fixfind \cup \fixtemp$
        \WHILE{$\coll \not = \emptyset$} \label{alg32:while2}
            \STATE $(\acto,\coll) \leftarrow InformationCombine(\acto,\coll)$ 
        \ENDWHILE
    \ENDWHILE
\RETURN $\fixfind$
...
For $\pairi \in \acta$, compute the Tarski sequence $\Phi_i^k \pairi$ until a stopping condition (as above) is triggered for some $k = \ell$. 
Then, do the following:
\begin{itemize}
\item[(a)] Set $\pairp = \Phi^{\ell}\pairi$ and add $w\pairp$ to $\forest$ with $c(w\pairp) = w\pairi$ and 
...
Then, (ii) for each $x \in X' \setminus \botx$ and $y \in \boty \setminus Y'$, add $(X' \setminus \{x\}, Y')$ and $(X', Y' \cup \{y \})$ to $\acto, \acta$, and $\act$.
...
\item For $j \in \mathcal{J}$, compute reachable set $\reach_j$ 
\end{enumerate}

For $j \in \mathcal{J}$, do the following to form new problems:

\begin{itemize}
\item[(a)] Add $\pairp = \lft \bigcap\limits_{v_k \in \reach_j} X_k, \bigcup\limits_{v_k \in \reach_j} Y_k \rt$ to $\actt$. 
\item[(b)] Add $w\pairp$ to $\forest$ with $c(w\pairp) = \{r(w_k): v_k \in \strongcomp_j\}$.
\end{itemize}

Set $\coll(i) = \emptyset$ for $i \in \mathcal{I}$. 
To check for new collisions, do the following:

For each $\pairp \in \actt$, if $\exists (X_k, Y_k) \in \act$ and $w_j \in c(w_i)$ such that $\pairp \peq (X_k, Y_k)$ and $(X_j, Y_j) \not \peq (X_k, Y_k)$, then add collision index $j$ to $\coll(i)$.
...
\emph{Discussion}: We briefly note the substantive differences from algorithm~\ref{mainalg} for finding all strict core outcomes.
Here, when forming new problems during InformationCombine, we replace the pointwise minima used in algorithm~\ref{mainalg} with the appropriate meet (greatest lower bound) for the lattice $2^X \times 2^X$.
...
Thus, after finding $(Z,W) \in \fix(\Phi)$, we still have to search the lattice $\{\pairp \pe (Z,W) \}$ strictly below $(Z,W)$, whereas, for algorithm~\ref{mainalg}, lemma~\ref{lemma:order} shows that we can stop immediately.
Because of this, subroutine~\ref{sub:infac2} has one fewer distinct stopping condition.
Aside from these modifications, the algorithm is structurally identical, with slightly different notation.

The following result is an easy extension of lemma~\ref{lemma:termination} and theorem~\ref{thm:correct}, the termination and correctness results for FindStrictCore.

\begin{thm}
When FindStable returns, $\fixfind = \fix(\Phi)$. 
In particular, $\stable = \{Z \cap W: (Z,W) \in \fixfind\}$, so the algorithm finds all stable allocations.
\end{thm}

\prf
The proof is a straightforward extension of the auxillary lemmas and termination and correctness proofs provided above, replacing pointwise minima with the lattice meet for $2^X \times 2^X$.
Here, we content ourselves with noting the substantive differences.
...
AGENDA
Write out similar version of informationcombine
Write out theorem 
Figure out inductive proof of guarantee


consult with Scott on changing everything to general
figure out how to prove guarantee on Q 
    how to insert this in other work / which lemma


write out stable allocation stuff 
    need to show that we can actually track all problems in Q
    figure out how chains work in this setting
        is there a relevant result e.g. about exp decrease?
    write out algorithm with Echenique style / more concise 
