...
\newcommand{\reach}{R}
\newcommand{\forest}{F}
\newcommand{\fixfind}{\wh{\mathcal{E}}
...
Let $\pre^*$ and $\pre_*$ the maximal and minimal allocations in $\prealloc$, and apply $T^2$ to each to find $\toppre = max \fix(T^2)$ and $\ul{\pre}$, respectively.
If $T\toppre = \toppre$ set $\fixfind = \{\toppre\}$ and terminate.
If $T\ul{\pre} = \ul{\pre}$ and terminate. 
Otherwise, set $\act = \acto = \bigcup\limits_{i =1}^m \{\toppre - e_i\}$ and run 

\begin{algorithm}
\caption{do this thing} 
\label{the alg}
\begin{algorithmic}
\REQUIRE $\acto$, $\act$
\ENSURE $\fixfind$
