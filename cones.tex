...
\]
and, by an abuse of notation, extend these functions to $Y\in 2^X$ by setting $C_a(Y) \equiv C_a(Y_a)$.
...
For condition~(\ref{convlemma:coll}) of the lemma about collision indices, it suffices to consider $w_{\ell}$ with $r(w_{\ell}) = w(\pre)$, because otherwise the statement follows directly from the inductive hypothesis, since $w(\pre)$ is the only root added to $\forest_n$.
...
First, we show that, for any problem collection $\acto$, InformationAcquire (subroutine 1) \ref{infac} terminates in finitely many iterations.
...
Using the map $p$ above, we can express $\actok$ as a disjoint union of the form $\actok = \cup_{\pre \in \actt} p\inv(\pre)$.
...
Note that, for all such $\pre_i \in \actc$, we add $w(\pre_i^f) = p(w_i)$ (line~\ref{alg1:tree2}), so that $r(p(w_i)) = r(w(\pre_i^f))$, which we denote by $w_k$.
...
\subsubsection{Generalized Information Sharing and Applications}
In this section, we briefly describe how the notion of information sharing as well as the collision digraph algorithm developed above can be extended to similar economic problems on more general complete lattices.
We demonstrate the utility of this generalization in two ways.
First, we briefly describe an information sharing algorithm for finding all pure strategy Nash equilibria in games with strategic complementarities (GSC), extending the algorithm pioneered in Echenique (2003).
Second, we show how these ideas subsume an information sharing based algorithm for more quickly finding all stable allocations.
...
\emph{Lattice Structure} - Fix a complete lattice $L$ with partial order $\peq$, and let $\point \sub L$ be a set of points that we are would like to find. 
For instance, these could be preallocations $\fix(T)$ corresponding to strict core outcomes in the lattice $\prealloc$ or stable allocations $\stable$ in the lattice $(2^X,\sub)$, where subsets are partially ordered by set inclusion. 
Let $\oper: L \to L$ be an isotonic operator such that $\point \sub \fix(\oper)$; that is, the fixed points of $\oper$ \emph{bound} $\point$.
It is easy to show that for any $x_i \in L$, where $i$ is some index, the set $\{x' \peq x_i\} \equiv L_i \sub L$ is also a complete lattice.
Suppose that isotone operators $F_i : L_i \to L_i$ can also be defined.

\emph{Economic Structure} - To complete the construction, we need to assume some economic structure. 
Specifically, suppose that fixed points of $\oper$ in the sublattic $L_i$ have $\fix(\oper) \cap L_i \sub \fix(\oper_i)$.
Guarantees of this form are surprisingly common in economics.
For finding strict core allocations in $\prealloc$, for instance, the guarantee is provided by lemma~\ref{lemma:contain}. 
This lemma is based on the very general observation that truncating agents' preference profiles can only shrink the available set of blocking allocations.
The corresopnding guarantee for the problem of finding all stable allocations in the lattice $(2^X, \sub)$ is similar, based on the observation of Hatfield and Kominers that substitutability is preserved under the coarsening of a contract language (see section xxxxx).
...
AGENDA
write connection / extension to other results 
