...
We develop a notion of information sharing in lattices, showing how lattice geometry and information sharing can be exploited to produce a relatively fast algorithm that returns the full list of core outcomes in this general model.
We show how to apply this technique to more general economic problems and, as an application, construct the first algorithm to find all stable allocations in bilateral matching with contracts and substitutable preferences. 
\end{abstract}
...
This model nests a great number of previously considered matching models.
...
There are a variety of results showing that, when preferences satisfy a ``substitutability'' condition, the set of stable matchings in matching with contracts takes the form of a lattice.
Usually, these results proceed by showing that substitutability implies the existence of an isotone operator on some lattice containing the stable matchings\footnote{see references}.
By iteration from a maximal element, one eventually arrives at an extremal stable matching.
Conversely, in the absence of substitutability, not only are stable matchings not guaranteed to exist, but the usual isotone operator is not guaranteed to find any that do exist\footnote{see references}.  
...
Specifically we show that, even in the absence of substitutability, it is possible to develop a relatively fast algorithm that either returns all \emph{core} allocations (or the null set if none exist). 
The core is a natural replacement for stability in the absence of substitutability guarantees; indeed, in many classical models such as the marriage problem, the core coincides with the set of stable matchings\footnote{ref roth and sotomayor}. 
When substitutabity does hold, we show how our technique can be used to find \emph{all} stable matchings, not just the extremal ones.

Lattice structures arise frequently in economics.
We show how lattice geometry can be combined with economic problem structure to develop a notion of information sharing in lattices, which allows us to more quickly compute objects of economic interest.  
The algorithm developed in this paper thus has independent economic value outside of matching theory. 

In what follows we briefly survey the related literature.
...
First, we give detail on our model and notation.
