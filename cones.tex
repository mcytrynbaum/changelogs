...
For the statements below, we work by induction on $\ell$, the total number of evaluations of subroutine~\ref{infac} and \ref{\infcomb}. 
...
about the structure of graphs returned from our 
...
First, we characterize the structure of $\roott(\forest)$. 
We claim that, for a distinct $i$, $\pre_i$ is processed at most once during subroutine~\ref{infac}. 
That is, $\pre_i$ is added to the active set $\acta$ at line~\ref{alg1:while2} at most once during a single execution of the subroutine. 
