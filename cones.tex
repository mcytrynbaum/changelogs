...
\emph{Case 3} Before constructing the collision digraph in subroutine~\ref{infcomb}, there is a transition $\pair_n \to \pair_{n + 1}$ when we add $i$ to $\coll(i)$. 
...
\emph{Case 4}: There is a transition $\pair_n \to \pair_{n + 1}$ in subroutine~\ref{infac} when we add collision index $j$ to $\coll_n(i)$ for some newly created problem $\pre_i \in \actt$ (line~\ref{alg2:coll2}).
...
\emph{Case 5}: There is a transition $\pair_n \to \pair_{n + 1}$ by adding a vertex to $\forest_n$ after processing the collision digraph $G$ on line~\ref{alg2:forest} of subroutine~\ref{infcomb}.
