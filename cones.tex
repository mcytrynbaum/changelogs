...
We will show by induction that for all $\pre \in \acto_{\ell}$, if $\pre'' \in \fix(\pre)$, then either $\pre'' \in \fixtemp_{\ell}$ or there exists $\pre' \in \acta_{\ell} \cup \actc_{\ell}$ with $\pre'' \peq \pre' \pre \pre$.
