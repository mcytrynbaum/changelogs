...
Let $\pre_i$ be an arbitrary problem with $r(w_i) = w_j$, and suppose that if $k \in \coll(i)$ then $\fix(\pre_j) \sub \fix(\pre_k)$. 
Let $G = (V,E)$ be the collision digraph and $S$ one of its strong components. Then we have
\begin{enumerate}
...
\end{enumerate}
\end{lemma}
Thus, by item~(\ref{lemma:graph:edge}), directed edges in $G$ reflect the relations between strict core allocations contained in different problem cones. 
As a consequence, item~(\ref{lemma:graph:strong}) shows that $G$'s maximal strongly connected components identify equivalent problems. 
The last statement shows that we can use breadth-first search, a fast digraph algorithm for computing reachable sets, to combine the information learned from different problems.
...
By the assumption that hypothesis, $\fix(\pre_{\ell}) \sub \fix(\pre_j)$. 
