...
Even more problematically, this fact shows that the monotonically decreasing Tarski sequence $(T^2)^k \pre$, which is the centerpiece of this approach, can never actually decrease.
...
The contract space is $X = \{(s_1,b), (s_2,b)^2, (s_2,b), \emptyset_{s_1}, \emptyset_{s_2}, \emptyset_b\}$, and we assume that all elements of $2^{X_a}$ not listed in the relations above are ranked below\footnote{Note that the specific order of allocations ranked below $\emptyset_a$ is irrelevant with respect to the evaluation of $\optwo$, since trivially $\emptyset_a \in W(\pre,a)$ for any $a$.} the single-agent contract $\emptyset_a$.
...
Finally, $\pre''(b)_{s_2} = \{(s_2,b)\}$, but $\{(s_2,b)\} \not \in \pre(s_2)$, so $\pre''$ is not coordinated.
However, we have shown that $\optwo(\pre'') = \pre''$, so $\fix(\optwo) \not \sub \coordpre$.
\eprf

\emph{Discussion}: These results, and the remark above, show that the fixed point approach to finding all core allocations is doomed to fail on the lattice $\prealloc$.
Similar results can be shown, for instance, for 
...
AGENDA
write maximal domain results and examples
...
open question - is there a lattice such that this works?
look: what does substituability have to do with core equivalence results?
    possible note above, since I constructed non-sub prefs
