...
As in Hatfield and Kominers, we may define an operator $\Phi: \lattice \to \lattice$ by $\Phi(Z,W) = (\Phi_D(W), \Phi_H(Z))$, where $\Phi_D(Z) = \{z \in X: z \in C_D(\{z \} \cup Z) \}$ and $\Phi_H(W) = \{z \in X: z \in C_H(\{z \} \cup W) \}$.
...
This lemma shows that finding all stable matchings $\stable$ is equivalent to finding all fixed points of the operator $\fix(\Phi)$.
By isotonicity of $\Phi$, the standard Tarski argument from previous sections shows that the sequence formed by $\Phi^k(X, \emptyset)$ is monotonically decreasing, and fixes at $(\ol{X} 
