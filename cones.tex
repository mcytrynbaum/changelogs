...
\draw[purple,->, ultra thick] (5,6) to [out=180, in=70] (3,5);
\draw[purple,->, ultra thick, bend left] (6,5) to (6,2);
...
\subsubsection{Full Algorithm} \label{fullalgo}
In this section, we state the full algorithm to find all strict core allocations $\strcore$. 
First, we need define some stopping conditions.
...
For Condition~(\ref{cond:fix1}), the Tarski sequence fixes, and we are forced to initialize new problems.
For Condition~(\ref{cond:collide1}), the problem $\lag \pre_i \rag$ ``collides'' with some problem in $\act$, so we stop the sequence and wait for the information sharing subroutine.
...
%\newcommand{infacstop2i}{(i)}
%\newcommand{infacstop2ii}{(ii)}
%\newcommand{infacstop2iii}{(iii)}
%\newcommand{infacstop3a}{(a)}
%\newcommand{infacstop3b}{(b)}
%%%%%%%
...
\emph{Case 1}: There is a transition $\pair_n \to \pair_{n +1}$ in subroutine ~\ref{sub:infac} when stopping condition (1) or (2) is triggered, and $w(\pre_i^f)$ is added to $\forest$ (line~\ref{alg1:tree1}). 
...
\emph{Case 2}: There is a transition $\pair_n \to \pair_{n +1}$ in subroutine~\ref{sub:infac} during processing of $\pre_i$, where stopping condition (4) is triggered and $w(\pre_i^f)$ is added to $\forest$ (line~\ref{alg1:tree2}). 
...
\emph{Case 3}: There is a transition $\pair_n \to \pair_{n +1}$ in subroutine~\ref{sub:infac} during processing of $\pre_i$, when stopping condition (3) is triggered and $w(\pre_i - e_j)$ is added to $\forest_n$ as an isolated root (line~\ref{alg1:addq}). 
...
\emph{Case 5}: There is a transition $\pair_n \to \pair_{n + 1}$ by adding a vertex $w(\pre)$ to $\forest_n$ while processing the collision digraph $G = (V,E)$ during subroutine~\ref{sub:infcomb} (line~\ref{alg2:forest}).
...
\emph{Case 6}: There is a transition $\pair_n \to \pair_{n + 1}$ in subroutine~\ref{sub:infac} when we add collision index $j$ to $\coll_n(i)$ for some newly created problem $\pre_i \in \actt$ (line~\ref{alg2:coll2}).
...
\emph{Discussion}: We briefly note the substantive differences between this algorithm and algorithm~\ref{alg:main} for finding all strict core outcomes.
...
AGENDA
deal with some discussion / tune
rewrite main algorithms / proofs and try to slim

TODO-------
add discussion sections, minus related literature
    beginning of strcore
    exploiting lattice geometry
