...
Combining the monotonicity of $\actok$ within the while loop at line~\ref{alg3:while2} with the monotonicity proof from our analysis of subroutine~\ref{infac}, we have shown that the sequence $\disto(\actok)$ formed by distinct states of $\acto$ within the outer while loop in FindStrictCore is monotone down.  

Now consider an evaluation of subroutine~\ref{infcomb} with input $\actok$. 
Then, by inspection, we must have $\actok \not = \emptyset$
...
discuss hwo each exits, then collate 
