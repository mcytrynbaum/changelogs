...
For Condition~(\ref{cond:collide}), the problem $\lag (X_i, Y_i) \rag$ collides with a problem in $\act$, so we stop the sequence and wait for the information sharing subroutine.
...
Then run algorithm~\ref{mainalg}, where we make the following modifications to each subroutine.
