...
\newcommand{\reach}{R}
\newcommand{\forest}{F}
\newcommand{\fixfind}{\widehat{\mathcal{E}}}
...
Let $\acta$ be the collection of preallocations of the form $\{S^k \pre: \pre \in \acto\}$ that, at any point during the algorithm, contains the current value of the Tarski sequence for each active subproblem.  
...
We successively iterate $\preo_i \to S_i^k \preo_i \equiv \pre_i$ until one of the following occurs \emph{stopping conditions} occurs
\begin{enumerate}
\item[stop1] $\pre_i \peq \ul{\pre} = \min \fix(T^2)$ 
\item $T_i \pre_i = \pre_i$
...
In case (3), $\pre_i$ is fixed by $T_i$, so by lemma xxxx, $\fix(T) \cap \{\pre' \pe \pre_i = \emptyset$.
That is, we can stop searching this problem.
In case (4), we have learned that $\fix(\pre_i) \sub \fix(\pre_j)$ for some other problem, so we stop work on $\preo_i$, waiting to combine its information with that of problem $\preo_j$ in the next stage of the algorithm. 
...
In general, we would like to spend more time in stage 2, since stage 2 improves the progress of the search by using fast ($\mathcal{O}\left(|V| + |E|)\right$) digraph algorithms, as opposed to the costly process (how costly xxxx) of computing the operator $T$, which is equivalent to checking whether an allocation strict core.  
...
Throughout, we maintain global variables $\forest$, a problem forest, and $\act$, a collection of preallocations corresponding to both active and inactive problem cone. 
 
\begin{algorithm}
\caption{InformationAcquire}  
\label{infac}
\begin{algorithmic}[1]
\REQUIRE $\acto$
    \STATE Set $\acta \leftarrow \acto$
    \STATE Set $\coll = \emptyset$
    \FOR{$\preo \in \acto$}   
        \IF[Stop redundant problems]{$\exists \pre' \in \acto$ s.t. $\preo \peq \pre'$}
            \STATE Remove $\pre$ from $\acta$
            \STATE Add $\pre'$ to $I(\preo)$
        \ENDIF
    \ENDFOR
    \WHILE{$\acta \not = \emptyset$}
        \FOR{$\pre \in \acta$} 
            \IF[Stopping Condition 1]{$T\pre = \pre$} 
                \STATE Remove $\pre$ from $\acto$ and $\acta$
                \STATE Set $\pref \leftarrow \pre$; Add $v(\pref)$ to $F$ with $c(v(\pref)) = v(\preo)$
            \ELSIF[Stopping Condition 2]{$T^2 \pre = \pre$}
                \STATE Remove $\pre$ from $\acto$ and $\acta$
                \FOR{$i = 1$ to $m$}
                    \STATE Add $\pre - e_i$ to $\acto$ and $\acta$
                \ENDFOR
                \STATE Set $\pref \leftarrow \pre$; Add $v(\pref)$ to $F$ with $c(v(\pref)) = v(\preo)$
            \ELSIF[Stopping Condition 3]{$\pre' \equiv T^2 \pre \peq \bopre$}  
                \STATE Remove $\preo$ from $\acto$ and $\pre$ from $\acta$ 
                \STATE Set $\pref \leftarrow \pre'$; Add $v(\pref)$ to $F$ with $c(v(\pref)) = v(\preo)$
            \ELSE[Stopping Condition 4]
                \STATE Set $\pre' = T^2 \pre$
                \FOR{$\pre'' \in Q$}
                    \IF{$\pre' \peq \pre''$ \AND $\preo \not \peq \pre''$} 
                        \STATE Remove $\pre$ from $\acta$
                        \STATE Set $\pref \leftarrow \pre'$; Add $v(\pref)$ to $F$ with $c(v(\pref)) = v(\preo)$ 
                        \STATE Add $\pre''$ to $\coll(\preo)$
                    \ENDIF
                \ENDFOR
            \ENDIF
        \ENDFOR
...
\begin{algorithm}
\caption{InformationCombine} 
\label{infcomb}
\begin{algorithmic}[1]
\REQUIRE $\acto$, $\coll$
    \STATE Set $V = E = \emptyset$
    \STATE Set $Q' = \emptyset$
    \STATE Set $\coll = \emptyset$
    \FOR{$\pre_i \in \acto$}
        \STATE Add $v_i$ to $V$
        \FOR{$\pre_j \in \coll(\pre_i)$}
            \STATE Add $(i,j)$ to $E$
        \ENDFOR
    \ENDFOR
    \STATE Set  $G = (V,E)$
    \STATE Compute strong components $\strongc = \{W_j\}_{j \in \mathcal{J}}$ using Tarjan's Algorithm  
    \FOR{$j \in \mathcal{J}$}
        \STATE Compute $\reach_j$, the reachable set of $W_j$
        \STATE Compute $\pre_{int} = \min\limits_{\pre: v(\pre) \in \reach_j} r(\pre')$
        \STATE Add $\pre' = \min(\pre_{int}, r(\pre))$ to $Q'$
        \STATE Add $w(\pre)$ to $F$ with $c(w(\pre)) = \{w_i: v_i \in W_j\}$
    \ENDFOR
    \FOR{$\pre_i \in Q'$} 
        \FOR{$\pre \in Q$}
            \IF{$\exists w_j \in c(w_i)$ s.t. $\pre_i \peq \pre$ \AND $\pre_j \not \peq \pre$}
                \STATE Add $\pre$ to $\coll(\pre_i)$
            \ENDIF
        \ENDFOR
    \ENDFOR
    \STATE Set $\acto \leftarrow \act$ 
\RETURN{$(\acto,\coll)$} 
\end{algorithmic}
\end{algorithm}

To initialize, let $\fixfind = \emptyset$. 
...
Otherwise, let $\act = \acto = \bigcup\limits_{i =1}^m \{\toppre - e_i\}$ and run the algorithm.
...
\ENSURE $\fixfind$
    \WHILE{$\acto \not = \emptyset$}
        \STATE $(\acto, \coll) \leftarrow InformationAcquire(\acto)$ 
        \WHILE{$\coll \not = \emptyset$}
...
\subsubsection{notes}
add something about using correct operator
can you cross a new cone on collation iteration? 
for clarity, w's and v's 
need to make all this index linking less opaque 
add something about liniking indices 
define the intersection set appropriately
let's take care of self intersection later i.e. combine isect with own information 
maybe we should produce intersection indices // yes we actually have to 
add notes about refs for final, initial problems
add removal code above 
a problem tree should correspond to a problem - notes 
can e.g. have notes after each subroutine
prealloc notation is not exactly correct
add something above about how the algorithm makes use of certain facts 
form new problems 
repeat 
explain repetition
