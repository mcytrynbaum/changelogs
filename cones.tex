                ...
                \ELSE
                    \FOR{$\pre_j \in Q$}
                        \IF[Stopping Condition 4]{$\pre' \peq \pre_j$ \AND $\pre_i \not \peq \pre_j$} 
                            \STATE Set $\pref \leftarrow \pre'$; Add $w(\pref)$ to $F$ with $c(w(\pref)) = w(\pre_i)$ 
                            \STATE Add $j$ to $\coll(i)$
                        \ENDIF
                    \ENDFOR
                \ENDWHILE
...
\REQUIRE $\acto$, $\coll$
    ...
    \ENDFOR
    \STATE Set  $G = (V,E)$
    \STATE Compute\footnote{Using Tarjan's Algorithm, for instance.} the strong components $\strongc = \{\strongcomp_j\}_{j \in \mathcal{J}}$
    \FOR[Use $G$ to Combine Information]{$j \in \mathcal{J}$}
        \STATE Compute reachable component $\reach_j$ 
        \STATE Compute $K_j = \{w_k \in F: \exists i\in \mathcal{I} \; s.t. \; w_k = r(w_i) \: and \: v_i \in R_j\}$
        \STATE Add $\pre = \min \{\pre_i: w_i \in K_j\}$ to $\actt$
        \STATE Add $w(\pre)$ to $F$ with $c(w(\pre)) = \{w_i: v_i \in \strongcomp_j\}$
    \ENDFOR
...
And now for some auxillary lemmas

\begin{lemma}
For any input $\acto$, InformationAcquire terminates
\end{lemma}
\prf
\eprf 

\begin{lemma}
For any input $\acto$, InformationCombine terminates
\end{lemma}
\prf
\eprf 
\subsubsection{notes}
