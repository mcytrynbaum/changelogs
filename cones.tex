        ...
        \STATE $\pre \leftarrow \pre_i$
            \WHILE{$\pre_i \in \acta$} 
                ...
                \IF[Stopping conditions 2 and 3]{$\pre' = \pre$ \OR $\pre' \peq \bopre$}
                    ...
                    \ELSIF{$\pre' = \pre$}
                        \FOR{$i = 1$ to $m$}
                            \STATE Add $\pre - e_i$ to $\acto$ and $\acta$ \label{alg1:addq}
                        \ENDFOR
                    \ENDIF
                \ELSE
                    \FOR{$\pre_j \in Q$}
                        \IF[Stopping condition 4]{$\pre' \peq \pre_j$ \AND $\pre_i \not \peq \pre_j$} 
                            \STATE Remove $\pre_i$ from $\acta$
...
Fix $\acta^{0}$ at the beginning of the for loop on line~\ref{alg1:for1}. 
Then $\pre \in \acta^{0}$, so, for at least one iteration of the loop~\ref{alg1:for1}, $d(\actak)$ decreases strictly.
Since $d(\actak)$ eventually fixes, the for loop at line~\ref{alg1:for1} is executed at most finitely many times. 
...
By forest construction in line~\ref{alg2:forest}, there is a surjection 
