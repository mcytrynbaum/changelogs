        ...
        \STATE $\pre \leftarrow \pre_i$
            \WHILE{$\pre_i \in \acta$} 
                ...
                \IF[Stopping conditions 2 and 3]{$\pre' = \pre$ \OR $\pre' \peq \bopre$}
                    ...
                    \ELSIF{$\pre' = \pre$}
                        \FOR{$i = 1$ to $m$}
                            \STATE Add $\pre - e_i$ to $\acto$ and $\acta$ \label{alg:addq}
                        \ENDFOR
                    \ENDIF
                \ELSE
                    \FOR{$\pre_j \in Q$}
...
\REQUIRE $\acto$, $\coll$
    ...
    \FOR[Use $G$ to Combine Information]{$j \in \mathcal{J}$}
        ...
        \STATE Compute tree indices $K_j = \{k \in F: \exists i\in \mathcal{I} \; s.t. \; w_k = r(w_i) \: and \: v_i \in R_j\}$
...
By finiteness, $d(\cdot)$ takes finitely many values on subsets of $\prealloc$.
Note that if $\acto \to \acto \setminus \{\pre\}$ at some point in the algorithm, then clearly $d(\acto)$ weakly decreases.
All changes to $\acto$ except for the change on line~\ref{alg:addq} are of this form
For line~\ref{alg:addq}, note that any $\pre \in \prealloc$ has $\|\pre\|_1 > \max_i \|\pre - e_i\|$.
Thus, the sequence $d(\acto)$ formed by evaluating at every state $\acto$ is monotone down. 
By finiteness as above, $\acto$ e
