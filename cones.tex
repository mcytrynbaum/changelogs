...
We are now ready to prove correctness using the lemmas above.
\begin{thm}
FindStrictCore finds all fixed points $\fix(T)$. 
\end{thm}
\prf
Consider
Following the general approach of Echenique and Yenmez, we show by induction that at any point during FindStrictCore, $\pre \in \fix(T)$ has either (i) $\pre \in \fixfind$ or (ii) there is a $\pre' \in \acto$ such that $\pre' \peq \pre$. 
