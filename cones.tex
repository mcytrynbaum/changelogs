...
Then run algorithm~\ref{mainalg}, where we make the following modifications to the InformationAcquire (subroutine 1) and InformationCombine (subroutine 2).

\begin{alg}[InformationAcquire]
...
\begin{itemize}
\item[(a)] Set $\pairp = \Phi^{\ell}\pairi$ and add $w(\pairp)f$ to $\forest$ with $c(w(\pairp)) = w(\pairi)$ and 
...
In this section, we give the full algorithm for finding all stable allocations in many-to-many two-sided matching, assume substitutability and irrelevance of rejected contracts.
The algorithm is a straightforward extension of algorithm~\ref{mainalg}, incorporating elements of the discussion above.

Set xxxxx
Remove all problems s.t. xxxxxx from sets xxxxx 
While Qa not empty, for prob in qa do
    compute the Tarski sequence until one of the stopping conditions (1) - (4) is reached 
    remove from Qa and add to graph
    do the following thing
        (2) add to found set 
        (3) spawn new problems and check for redundancies, add to graph 
        (4) build collision indices 

this is much better - enumerate and reference enumeration in proof 
...
write out pseduo alg structure 
write out full algorithm for a general lattice
