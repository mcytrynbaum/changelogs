...
\begin{subroutine}[InformationAcquire]
...
\begin{subroutine}[InformationCombine]
...
\emph{Discussion}: We briefly note differences from algorithm~\ref{mainalg} above for finding all strict core outcomes.
As discussed in section~\ref{section:general}, when forming new problems during InformationCombine, we replace the pointwise minima of algorithm~\ref{mainalg} with the appropriate meet (greatest lower bound) for the lattice $2^X \times 2^X$.
For the order ``$\peq$'', the meet is given by intersections in the first component and unions in the second.
Note that, for this algorithm, the number of subproblems initialized when a Tarski sequence fixes is point-dependent.
This is necessary because $2^X \times 2^X$ has a less regular structure than the Euclidean lattice $\prealloc$ of section~\ref{section:strcore}
Finally, for the case of stable allocations, there is no equivalent of lemma~\ref{lemma:
