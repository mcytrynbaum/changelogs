...
\item There exists a $w_k \in F$ such that $w_i, w_j \in c(w_k)$, then $\fix(\pre_i) = \fix(\pre_j)$. \label{def:forest2}
\item If $w_i = p(w_j)$, then $\fix(\pre_j) = \fix(\pre_i)$. \label{def:forest3}
\end{itemize} 
...
We inductively assume the existence of such a forest (proof in section~\ref{fullalgo}). 
        ...
        \STATE $\pre \leftarrow \pre_i$
            \WHILE{$\pre_i \in \acta$} 
                ...
                \IF[Stopping conditions 2 and 3]{$\pre' = \pre$ \OR $\pre' \peq \bopre$}
                    ...
                    \ELSIF{$\pre' = \pre$}
                        \FOR{$i = 1$ to $m$}
                            \IF[Add new problems]{there is no $\pre'' \in \acto$ with $\pre - e_i \peq \pre''$ }
...
For any set $E \sub \prealloc$, we define $\disto(E) = \max_{\pre \in E} \|\pre\|_1$, where we identify $\prealloc$ with a grid as above\footnote{For $\x \in \mr^m$, $\|x\|_1$ is the standard 1-norm.}.
...
For line~\ref{alg1:addq}, note that it is always the case that $\|\pre\|_1 > \max_i \|\pre - e_i\|$, so, in fact, if $\actak \to \acta^{k+1}$ on line~\ref{alg1:addq} then $\disto(\acta^{k+1}) \leq \disto(\actak)$. 
This shows that $\disto(\actak)$ is a monotonically decreasing sequence. 
...
We will show the sequence of positive real numbers $\distt(\actok)$ is monotone decreasing.
...
Therefore, by disjointness of strong components $\{\strongcomp_j\}_{j \in \mathcal{J}}$, there exists a surjection $p: \acto \to \actt$ defined by $p(\pre_i) = \min_{k \in K_j} \pre_k \in \actt$. 
This also shows, by construction, that for any $\pre_i \in \actt$, $p\inv(\{\pre_i\}) = \{\pre_j: w_j \in c(w_i)\}$. 
...
By the surjection $p$ above, we can write $\acto$ as a disjoint union $\acto = \cup_{\pre \in \actt} p\inv(\pre)$. 
So we can calculate $\disto(\actt) = \max_{\pre_i \in \actt} \|\pre_i\|_1 \leq \max_{\pre_i \in \actt} \disto(p\inv(\pre_i)) = \disto(\acto)$. 
Similarly, we can compute $\distt(\acto) = \sum_{\pre \in \actt} \distt(p\inv(\pre)) \geq \sum_{\pre \in \actt} \distt(\{\pre\}) = \distt(\actt)$ by surjection and the discussion above.
Then we have shown that $\disto(\acto^{k+1}) \leq \disto(\actok)$ and $\distt(\acto^{k+1}) \leq \distt(\actok)$ for any $k$. 

In particular, the sequence $\distt(\actok)$ constructed above is monotonically decreasing (and bounded below by 0), so it converges in finite time, again using finiteness of $\prealloc$. 
Suppose that for some $k$, with $\actok$ the input to subroutine $\ref{infcomb}$, we have $\distt(\actok) = \distt(\acto^{k+1})$.
By the previous discussion, $\distt(p\inv(\pre_i)) \geq \distt(\{\pre_i\}$ for $\pre_i \in \actt$, so each of these must actually be equality. 
Since, in addition, $p(\pre_j) \peq \pre_j$ for $\pre_j \in \actok$, each of these relations must actually be equality, with $p\inv(\pre_i) = \{\pre_i\}$ for any $\pre_i \in \actt$.
Then $\actok = \actt = \acto^{k+1}$. 
This argument shows that the \emph{sequence of sets} $\actok$ actually converges in finite time also.
...
By construction $\{j: \pre_j \in p\inv(\pre_i)\} = \{j: w_j \in c(w_i)\}$.  
...
Combining the monotonicity of $\actok$ within the while loop at line~\ref{alg3:while2} with the monotonicity proof from our analysis of subroutine~\ref{infac}, we have shown that the sequence $\disto(\actok)$ formed by distinct states of $\acto$ within the outer while loop in FindStrictCore is monotonically decreasing.  

Now consider an evaluation of subroutine~\ref{infcomb} with input $\acto$. 
By inspection, $\acto \not = \emptyset$. 

\emph{Case 1}: The collection $\actc$ returned by the subroutine is empty.
Then clearly we have $\disto(\acto^{k+1}) = \disto(\actc) < \dist(\actok)$. 

\emph{Case 2}: $\actc \not = \emptyset$.
Consider $\pre_i \in \actc$.
Since $\pre_i$ was added to $\actc$, line~\ref{alg1:coll} was executed, so there exists $k \geq 0$ such that $(T_i^2)^k \pre_i \not = (T_i^2)^{k+1}$. 
In particular, by monotonicity of the Tarski sequence for $\lag \pre_i \rag$, we must have $\pre_i^f \pe \pre_i$ strictly. 
Note that, for all such $\pre_i \in \actc$, by line~\ref{alg1:tree2}, $r(w_i) = w(\pre_i^f) \equiv w_k$ for some $k \in \mathcal{I}$. 
Denote $\acto' = \actc$, which is an input for subroutine~\ref{infcomb}.
Suppose that $W_j$ is the strong component containing $v_i$. 
By line~\ref{alg2:selfref} and line~\ref{alg2:rootbuild}, we have $(v_i,v_k) \in E$ for the collision digraph built during this subroutine execution, so $v_k \in R_j$. 
Therefore, $\pre' = \min_{\ell \in K_j} \pre_{\ell} \peq \pre_k \pe \pre_i$. 
Using the surjection $p:\acto' \to \actt$ above, we have $p(\pre_i) = \pre'$ for each such $\pre_i$. 
Then we can calculate as before $\disto(\actt) = \max_{\pre_i \in \actt} \|\pre_i\|_1 < \max_{\pre_i \in \actt} \disto(p\inv(\pre_i)) = \disto(\acto)$, where the inequality is strict. 
Since $\acto \leftarrow \actt$ when this subroutine returns, we have shown that, in both cases above, the sequence $\disto(\actok)$ decreases strictly during a single execution of the outer while loop in FindStrictCore. 
...
Next, we develop some basic graph theory results for problem forests. 
...
\item Adding a vertex $w$ and directed edges $(w,w')$, where $w' \in \roott(\forest)$. \label{forest:child}
\end{enumerate}
Then $\forest'$ is also a rooted forest. 
The reachable set $\reach(\forest,w)$ is unchanged by the modification $\forest \to \forest'$. 
Finally, the depth $\depth(w,\forest)$ of a vertex $w \in \forest$ weakly increases during a modification of the above form. 
\end{lemma}
...
That is, $\{t_i\}$ is the collection of maximal connected components of $\forest$.
For case (1), no edges were added, so the new collection of maximal connected components is now $\{t_i\} \cup \{w\}$. 
Each $t_i$ is still a rooted tree by assumption. 
$\{w\}$ is also trivially a rooted tree with root $w$. 

For (2) above, let $w$ be the added vertex and $J$ the subset of tree indices such that edge $(w,\roott(t_i))$ is added to $\forest$.
Let $i \in J$. 
Because each such $t_i$ is connected and connectedness is transitive, then $t' = \cup_{i \in J} t_i$ is a connected component in $\forest'$. 
Then, clearly, the connected components of $\forest'$ are $\{t_i\}_{i \not \in J} \cup \{t'\}$ . 
...
Next, we prove the statement concerning reachable sets. 
Case~(\ref{forest:solo}) above is clear, since no edges are added.  
For case~(\ref{forest:child}), let $w \in t$ for some tree in $\forest$. 
If there is a new path from $w \to w'$, $w' \not \in \reach(\forest,w)$, it uses one of the added edges $(w_1,w_2)$. 
Then, apparently, there exists a path $w \to w_1$ in $\forest'$. 
But, as shown above, $w_1$ is a root in the new forest $\forest'$, so there exists no edge of the form $(x,w_1)$. 
This is a contradiction, so it must be that $\reach(w,\forest) = \reach(w,\forest')$.

The final statement concerning vertex depth is clear. 
...
For any distinct $\pre \in \prealloc$, $\pre$ is processed (the Tarski sequence for $\lag \pre \rag$ is evaluated) at most once.
...
If $\pre_i$ is processed twice, then clearly $\pre_i$ is added to $\acta$ twice. 
Since $\acta \sub \acto$, and elements are \emph{only added} (never removed) to $\acto$ after line~\ref{alg1:redundant}, $\pre_i \in \acto$ when it is added to $\acta$ the second time. 
But we check for redundancy before we add a program in line~\ref{alg1:addq}, so this is a contradiction. 
Then any distinct preallocation is processed at most once. 

For the other lemma statements, we work by induction on $\ell$, the total number of evaluations of subroutine~\ref{infac} and \ref{\infcomb}. 
For $\ell = 0$ the statements are trivially true. 
Then assume that this holds up to $\ell = n$. 
There are several cases. 

\emph{Case 1}: Either subroutine~\ref{infac} evaluates directly after initialization or after subroutine~\ref{infcomb}. 
Let $\acto'$ and $\forest'$ denote the states of $\acto$ and $\forest$ at the beginning of this subroutine. 
If the subroutine executes following initialization, clearly $w(\pre) \in \roott(\forest')$ for $\pre \in \acto'$ by construction.
Otherwise, this holds by the inductive hypothesis and (\ref{lemma:roots:comb}) above. 
Note that each $\pre$ added to $\acto$ during this subroutine is initialized as an isolated root. 
Then we have shown that, at any time before $\pre$ is processed (the code starting at line~\ref{alg1:for2} is evaluated for $\pre \in \acta$), $w(\pre) \in \roott(\forest)$. 
Note that only edges of out-degree $1$ are ever added to $\forest$ during the subroutine. 

Suppose that for $\pre_i \not = \pre_j \in \acto$, $r(w_i) = r(w_j) = w$. 
Then there are directed paths $w \to w_i$ and $w \to w_j$. 
The path $w \to w_i$ can be represented as a sequence of vertices $w = w^1,w^2,\hdots, w^k = w_i$ and similarly for $w_j$. 
...
By minimality, the paths are identical up to this point. 
Then $w' = w_{k' - 1}$ has out-degree $\geq 2$, so $w'$ cannot have been added during the algorithm, so $w'$ is a vertex in the initial problem forest $\forest'$. 
Then $w' \in t$, one of the maximal connected components of $\forest'$. 
Let $\forest''$ denote the state of the graph $\forest$ when subroutine~\ref{infac} returns. 
Note that $\forest''$ is obtained from $\forest'$ by adding vertices and edges as in lemma~\ref{lemma:forest}. 
Then $\forest''$ is also a rooted forest and, by the lemma, $\reach(w',\forest) = \reach(w',\forest'')$, since there is a finite chain of edge and vertex additions taking $\forest' \to \forest''$, and the reachable set of $w'$ is fixed during each of these modifications.  
Since $w_i \not = w_j$, they cannot both be vertex $w'$. 
Then at least one of them, say $i$, has $\depth(w_i, \forest') \geq 1$
By lemma~\ref{lemma:forest} again, $\depth(w_i, \forest) \geq \depth(w_i, \forest') \geq 1$  for any state of the forest $\forest$. 
...
\emph{Case 2}: Subroutine~\ref{infcomb} executes directly after subroutine~\ref{infac}. 
As before, let $\forest'$ denote the state of $\forest$ at initialization and $\forest''$ the state of the forest when the subroutine returns. 
We show that $w(\pre) \in \roott(\forest'')$ for all $\pre$ in the returned set $\actt$. 
By definition, the strong components $\{\strongcomp_j\}_{j \in \mathcal{J}}$ are disjoint. 
Then if $j \not = j'$ with $v_k \in \strongcomp_j$, $v_{k'} \in \strongcomp_{j'}$, so $r(w_k) \not = r(w_{k'})$. 
Then we must have $\{r(w_k): v_k \in \strongcomp_j\} \cap \{r(w_k): v_k \in \strongcomp_{\j'}} = \emptyset$. 
Fix an order on $\mathcal{J}$, which can be extended to problems $\pre$ added to $\actt$ by letting $\pre_{j_1}$ be the problem added while processing strong component $j_1$. %begin cut - too mathsy? this should be obvious...
Define $R(j) = \{r(w_k): v_k \in \strongcomp_j\}$ to be the component roots of index $j$. 
Call an index $j_i$ \emph{processed} if $\pre_{j_i}$ has already been added to $\actt$. 
The problem forest $\forest$ is modified as we add new vertices, so roots change. 
However, we can show by induction on the number of processed problems $n'$ that if $j$ is unprocessed, $R(j)$ does not change during this procedure. 
The base case $n' = 0$ is trivial. 
Then assume this holds up to $n' = k$. 
Suppose we process index $j$ at $n' = k+1$ and let $J$ the set of unprocessed indices. 
All component roots $R(j)$ for $j \in J$ are unchanged up to this point by induction, so they are all disjoint. 
Then adding a new root $w(\pre_j)$ with $c(w(\pre_j)) = R(j)$ cannot change any of the other component root sets $R(\ell)$ for $\ell \in J \setminus \{j\}$.
This completes the induction. 
In particular, since $w(\pre) \not \in \forest$ at the beginning of the subroutine, then $w(\pre)$ is in no root set when $\forest = \forest'$, so no edge of the form $(w', w(\pre))$ is ever added during this procedure. 
Then each added tree vertex $w(\pre)$ is still a root in $\forest''$, when the subroutine returns, so we have shown (\ref{lemma:roots:comb}) above for this case. 

\emph{Case 3}: Subroutine~\ref{infcomb} runs after an execution of subroutine~\ref{infcomb}.  
