...
In this paper\footnote{I am very grateful to Scott Kominers for continual guidance on this project. This work was completed with the help of an Economic Design Summer Fellowship from the Harvard Society of Fellows.}, we extend an algorithm pioneered in Echenique and Yenmez (2013) to find \emph{all} strict core allocations in a highly general model of multilateral many-to-many matching with contracts.
...
Note that this result significantly generalizes the corresponding lemma in Echenique and Yenmez (2013) and also subsumes the main result of Kojima (2015). 
...
In this section, we generalize constructions from Echenique and Yenmez (2013) showing the the fixed points of the squared operator $T^2$ form a lattice. 
...
These results are an almost direct extension of lemmas 4.x through 4.y of Echenique and Yenmez (2013) .
...
The rest of the lemmas follow from straightforward extensions of the work in Echenique in and Yenmez (2013). 
...
\eprf
The following lemmas follow exactly as in Echenique and Yenmez (2013), using the antitonicity of $T$. 
...
Our algorithm builds upon the original approach in Echenique and Yenmez (2013).
...
In this way, we will eventually find the full set of strict core outcomes, directly extending the Echenique and Yenmez (2013) algorithm to the present setting.  
...
\prf
Note that a graph $G = (V,E)$ is a tree\footnote{Gross and Yellen (2005) p.116} if and only if $G$ is connected and $|E| = |V| - 1$. 
...
In particular, FindStrictCore and the original Echenique and Yenmez (2013) algorithm are incommensurable. 
...
For instance, this is the case in the model of many-to-one matching with preferences over colleagues considered considered in Echenique and Yenmez (2013) \footnote{something about other examples of this xxxx}.
...
We briefly show how the above discussion can be used to incorporate information sharing in the Echenique (2003) algorithm for finding all pure strategy Nash equilibria in GSC. 
...
Lemma 2 of Echenique (2003) shows that this map is isotone on $S$.
...
Lemmas 3 and 4 of Echenique (2003) show that the guarantee $\fix(\oper) \cap S_r \sub \fix (F_r)$ of the form (E) above holds for this family of operators $\oper_r$ on sublattices $S_r \sub S$, showing that the information sharing algorithm above may be applied.
...
As in Hatfield and Kominers (2015), we may define an operator $\Phi: \lattice \to \lattice$ by $\Phi(Z,W) = (\Phi_D(W), \Phi_H(Z))$, where $\Phi^D(Z) = \{z \in X: z \in C_D(\{z \} \cup Z) \}$ and $\Phi^H(W) = \{z \in X: z \in C_H(\{z \} \cup W) \}$.
...
\begin{lemma}[Hatfield and Kominers (2015), Lemma 1] \label{lemma:hk}
...
By Proposition 4 of Hatfield and Kominers (2015), substitutability is preserved under a coarsening of the contract language to any $X' \sub X$.
...
\subsubsection{Discussion} 
We briefly note the substantive differences between this algorithm and algorithm~\ref{alg:main} for finding all strict core outcomes.
...
\prf
This is a straightforward extension of the auxiliary lemmas and correctness proof provided above, using lemma~\ref{lemma:hk} from Hatfield and Kominers (2015) to get all of $\stable$ from the fixed point set $\fix(\Phi)$.
