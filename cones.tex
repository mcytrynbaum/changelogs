...
\item If $v_i = p(v_j)$, then $\pre_i \pre \pre_j$; in particular, $\fix(\pre_i) \sub \fix(\pre_j)$. 
\end{itemize} 
                ...
                \ELSE
                    \FOR{$\pre_j \in Q$}
                        \IF[Stopping condition 4]{$\pre' \peq \pre_j$ \AND $\pre_i \not \peq \pre_j$} 
                            ...
                            \STATE Add $\pre_i$ to $\actc$; Add $j$ to $\coll(i)$ \COMMENT{Build collision set} \label{alg1:coll}
...
Therefore, the outer while loop of FindStrictCore executes only finitely many times, and the algorithm terminates. 
\eprf 
...
adapt mon proof of subroutine 1 
plan for rest of proof 
    show that transitions between subroutines are strict
        done
