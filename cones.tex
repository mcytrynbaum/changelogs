...
\newcommand{infcombgraph1}{(1)}
\newcommand{infcombgraph2}{(2)}
\newcommand{infcombgraph3}{(3)}
\newcommand{infcombcomba}{(a)}

\newcommand{infacred1}{(i)}
\newcommand{infacred2}{(ii)}
...
\emph{Stopping Conditions}:
As above, we let $\act$ be a collection of problems, which we think of as previously solved problems that still contain information useful for finding $\fix(T)$. 
...
Condition~(\ref{cond:bottom1}) corresponds to the problem exiting the lattice $\fix(\Phi)$. 
...
\emph{Initialization} To initialize the algorithm, set $\fixfind = \emptyset$ and $F = (\emptyset,\emptyset)$. 
Apply $T^2$ iteratively to each of $\pre^*$ and $\pre_*$, the maximal and minimal allocations in $\prealloc$, to find $\toppre = \max \fix(T^2)$ and $\ul{\pre} = \min \fix(T^2)$, respectively.
If $T\toppre = \toppre$ set $\fixfind = \{\toppre\}$ and terminate.
...
\begin{subroutine}[InformationAcquire] \label{sub:infac1}
Initialize $\acta = \acto$, and set $\fixtemp = \actc = \coll = \emptyset$.
...
\begin{subroutine}[InformationCombine] \label{sub:infcomb1}
