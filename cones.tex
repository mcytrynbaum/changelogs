...
\prf
This is a straightforward extension of the auxillary lemmas and correctness proof provided above. 
Here, we just note the substantive differences.

In the analysis for algorithm~\ref{mainalg}, we use the lattice meet for $\prealloc$, which we think of as a sublattice of $\mr^m$.
...
For finding stable matchings, every such minimum must be replaced by the lattice meet for $\lattice = 2^X \times 2^X$.
...
Finally, as noted in the discussion above, there is an extra stopping condition in algorithm~\ref{mainalg} corresponding to the guarantee from lemma~\ref{lemma:order} that no two points $\pre, \pre' \in \fix(T^2)$ can be compared under $\peq$.
...
write out stable allocation stuff 
    ...
    possibly add a guarantee on doing better than greedy search
