...
\end{prop}
\prf
To begin, we prove an important fact that will be used repeatedly.
...
Since $Y_a = \pre(a)$, then in fact $Y_b \geq_b \pre(b)$ holds \emph{for all} agents $b \in d(Y)$. 
...
Therefore, $U(\pre,a_1) = \{Y \in 2^X: Y_{a_1} = \pre(a_1), \: Y_b \geq \pre(b)\}$ is non-empty, so there exists an allocation 

\[
Y \in \argmax_{Z \in U(\pre,a_1)} |d(Z)|
\]

Let $A_1 = d(Y)$. If $A_1 = A$, we are done, since then by the construction above $Y_a = \pre(a)$ for all $a \in A$, so $\pre = \pre_Y$. 
...
We also have $d(S) = \bigcup_{b \in A_2} d(\pre(b)) = \bigcup_{b \in A_2} d(Z_b) \subset d(Z)$, so $d(S) \subset A_1^c \cap d(Z) = A_2$.
Clearly $b \in d(\pre(b))$ for all $b \in A_2$, so $A_2 \sub d(S)$. Then $A_2 = d(S)$. 
Set $W = Y \cup S$. We have now shown that 

\begin{enumerate}
\item $A_2 \not = \emptyset$ and $A_2 \cap A_1 = \emptyset$. 
\item $W_b = Y_b = \pre(b)$ for $b \in A_1$; in particular, $W_a = \pre(a)$. 
\item $W_b = S_b = \pre(b)$ for $b \in A_2$.
\end{enumerate}

Then apparently $W \in U(\pre,a)$ as defined above. However, by construction $|d(W)| > |d(Y)|$, which contradicts our original choice of $Y$.
