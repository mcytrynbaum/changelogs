        ...
        \FOR{$\pre \in \acta$} 
            \IF{$T_i^2 \pre_i = \pre_i$}
                ...
                \ELSE[Stopping Condition 2]
                    ...
                    \ENDFOR
                \ENDIF
            \ELSIF[Stopping Condition 3]{$\pre' \equiv T_i^2 \pre_i \peq \bopre$}  
                \STATE Remove $\preo$ from $\acto$ and $\pre$ from $\acta$ 
                \STATE Set $\pref \leftarrow \pre'$; Add $v(\pref)$ to $F$ with $c(v(\pref)) = v(\preo)$
            \ELSE
                \STATE Set $\pre' = T^2 \pre$
                \STATE Add $\pre'$ to $\acta$; Remove $\pre$ from $\acta$
                \FOR{$\pre'' \in Q$}
                    \IF{$\pre' \peq \pre''$ \AND $\preo \not \peq \pre''$} 
                        \STATE Remove $\pre$ from $\acta$
                        \STATE Set $\pref \leftarrow \pre'$; Add $w(\pref)$ to $F$ with $c(w(\pref)) = w(\preo)$ 
...
\begin{algorithm}
\caption{Find All Strict Core Allocations} 
\label{mainalg}
\begin{algorithmic}[1]
\REQUIRE $\acto$, $\forest$
    \WHILE{$\acto \not = \emptyset$}
...
\subsubsection{notes}
look for uniqueness problems in the definition of preo 
