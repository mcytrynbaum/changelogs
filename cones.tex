...
Let $\fix(T)$ denote the fixed points of $T$, and define $\fecon(T) = \{Y \in 2^X: \, \pre_Y \in \fix(T)\}$, the collection of allocations $Y$ whose corresponding preallocation $\pre_Y$ is fixed by $T$. 
With these definitions, we have a simple result
...
This result shows that an algorithm that finds all $\pre \in \fix(T)$ will alsofind all strict core matchings. 
However, if there are \emph{uncoordinated} matchings that are also fixed by $T$, such an algorithm may would return extraneous preallocations not associated with any strict core matching.
Fortunately, there are no such prealloacations
...
These results are an almost direct extension Lemmas 4.x through 4.y of Echenique and Yenmez. We show the first result to give a sense of the general flavor, and refer the reader to Echenique and Yenmez for the rest.
