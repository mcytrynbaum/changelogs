...
\emph{Discussion}: talk about main algorithmic advantages, how this is incommensurable with E and Y and depends on the structure of the lattice (but there may be a prob. type result to be had). xxxxxxxxxx
...
Also, (ii) for each $x \in X' \setminus \botx$ and $y \in \boty \setminus Y'$, add $(X' \setminus \{x\}, Y')$ and $(X', Y' \cup \{y \})$ to $\acto, \acta$, and $\act$.
...
\emph{Discussion}: We briefly note the substantive differences between this algorithm and algorithm~\ref{mainalg} for finding all strict core outcomes.
...
\prf
This is a straightforward extension of the auxillary lemmas and correctness proof provided above, using lemma~\ref{lemma:hk} from Hatfield and Kominers to get all of $\stable$ from the fixed point set $\fix(\Phi)$.
Here, we just note the substantive differences between the proofs.

In the analysis for algorithm~\ref{mainalg}, we think of $\prealloc$ as a sublattice of $\mr^m$.
