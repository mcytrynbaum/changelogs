...
which can be thought of as the collection of allocations $Z \in 2^{X_a}$ corresponding to a ``block'' $Y \supseteq Z$ that agent $a$ could successfully propose to the coalition $d(Y)$.
...
\emph{Discussion}: Note that, by definition of the set $W(\pre,a)$, we must have $\optwo \pre \suq \pre$ for \emph{any} $\pre \in \prealloc$.
...
This fact also shows immediately that the monotonically decreasing Tarski sequence $(T^2)^k \pre$ which our algorithm relies on can never actually decrease.
The following example shows that the structure results for the operator $T$, which the fixed point approach relies on, may fail for the operator $\optwo$ even for two-sided, matching with bilateral contracts.
...
We let $(i,j)$ denote the contract where $i$ trades with $j$.
Suppose that agents have preferences as follows:
\begin{align*}
s1:& \{(s_1,b)\}, \emptyset_{s_1} \\
s2:& \{(s_2,b), (s_2,b)\}, \{(s_2,b)\}, \emptyset_{s_2} \\
b:& \{(s_1,b), (s_2,b) \}, \{(s_2,b),(s_2,b)\}, \{(s_2,b)\}, \{(s_1,b)\}, \emptyset_b
\end{align*}

Where the contract space is $X = \{(s_1,b), (s_2,b), (s_2,b), \emptyset_{s_1}, \emptyset_{s_2}, \emptyset_b\}$.
Thus, 
