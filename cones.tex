...
\item If $w_i = p(w_j)$, then $\fix(\pre_j) \sub \fix(\pre_i)$. \label{def:forest3}
\end{itemize} 

Thus, siblings in a problem forest share fixed points of $T$, and the fixed points of a child are contained in the fixed points of its children.
...
First, we give a definition and some useful auxillary lemmas. 
...
Let $\forest_0$ be a rooted forest. 
Suppose that $\forest$ is obtained from $\forest_0$ in one of the following ways 
\begin{enumerate}
\item Adding an isolated vertex to $\forest_0$ \label{forest:solo}
\item Adding a vertex $w$ and directed edges $(w,w')$, where $w' \in \roott(\forest_0)$. \label{forest:child}
\end{enumerate}
Then $\forest$ is also a rooted forest.
\end{lemma}

\prf
Note that a graph $G = (V,E)$ is a tree\footnote{Gross and Yellen xxxx} if and only if $G$ is a connected graph such that $|E| = |V| - 1$. 
Consider $\forest_0$ and let $\{t_i\}_{i \in I}$ be its constituent rooted trees. 
We let $V(t)$ and $E(t)$ denote the vertices and edges, respectively, of a tree $t$. 
That is, $\{t_i\}$ is the collection of maximal connected components of $\forest_0$.
...
For (2) above, let $w$ be the added vertex and $J$ the subset of tree indices such that edge $(w,\roott(t_i))$ is added to $\forest_0$.
Let $i \in J$. 
...
Then there is a directed path $w \to w'$, which is unique because $t'$ is cycle-free. 
Then $t'$ is a tree rooted at $w$.
\eprf

Next, we give a simple lemma, showing how we can use problem forests to track fixed point relations. 
...
Then $\fix(\pre_i) \sub \fix(\pre_j)$. 
\end{lemma}

\prf 
By assumption, $t$ is a rooted tree, so there is a unique directed path of the form $w_j = w^0 \to w^1 \to \hdots w^n = w_i$ from the root to $w_i$. 
Then, for each vertex in the path is related by $p(w^k) = w^{k - 1}$ for $1 \leq k \leq n$. 
Then, since $\forest$ is a problem forest by assumption, we get a chain of inclusions $\fix(\pre_{k-1}) \sub \fix(\pre_k)$ for $1 \leq k \leq n$, showing that, as claimed, $\fix(\pre_i) \sub \fix(\pre_j$. 
\eprf 
...
\emph{Case1}: There is a transition $\pair_n \to \pair_{n +1}$ in algorithm~\ref{infac} when a stopping condition is triggered and $w(\pre_i^f)$ is added to $\forest$ at line~\ref{alg1:tree1}. 

First, note that any $\pre \in \acto$ is added either during initialization or during an execution of subroutine~\ref{infcomb} immediately preceding subroutine~\ref{infac} or during the subroutine at line~\ref{alg1:addq}.  
...
This modification satisfies condition~\ref{forest:child} of lemma~\ref{lemma:forest} above, so $\forest_{n+1}$ is still a rooted forest by the lemma above. 
...
Combining this with lemma xxxx above, $\fix(\pre_i) = \fix(T) \cap \{\pre'' \peq \pre_i\} = \sub \fix(T_i) \peq \pre' = \pre_i^f$. 
Since $w(\pre_i^f) = p(w_i)$, we have shown that $\fix(\pre_i) \sub \fix(p(\pre_i))$ for the only \emph{new} parent-child relation added to $\forest$. 
...
By the exact same arguments used in case 1, $F_{n + 1}$ remains a problem forest for $\act$ after this modification. 
We need only check item~(\ref{convlemma:coll}) of our inductive hypothesis above.
$\coll_n(i) = \emptyset$, since $I = \emptyset$ at subroutine initialization, and each preallocation is processed at most once, as above. %again, break this off as a lemma%
For this modification $\coll_n \to \coll_{n + 1}$, we have that $\pre_i^f \peq \pre_j \in \act$, and $j$ is added to $I_n(i)$, so $I_{n + 1}(i) = \{j\}$. 
By the note at the beginning of case 1, $w(\pre_i) \in \roott(\forest)$. 
Since we add $w(\pre_i^f) = p(w_i))$, in fact, $w(\pre_i^f) = r(w_i)$. 
Note that $\pre_i^f \peq \pre_j$ by the stopping condition for this case.  

Note that, $\coll_n(\ell) = \coll_{n + 1}(\ell)$ for all $\ell \not = \coll$.
Fix such an $\ell$, and let $k \in \coll(\ell)$. 
By our inductive hypothesis, for $\forest = \forest_n$, we have $r(w_{\ell}) = w_j$, $\fix(\pre_j) \sub \fix(\pre_k$. 
If, with $\forest = \forest_{n + 1}$, we still have $r(w_{\ell}) = w_j$, we are done by induction. 
If not, then we must now have $r(w_{\ell}) = w(\pre_i^f)$. 
But $\ell \not = i$, so $w_{\ell}$ is a descendant of $w_i$.
By construction, all elements $\pre \in \acto$ have $w(\pre) \in \roott(\forest_0)$ or $w(\pre)$ is initialized as an isolated root, so this descendant relation is impossible.
Then condition~(\ref{convlemma:coll}) holds and we are done. 

\emph{Case 3}: There is a transition $\pair_n \to \pair_{n + 1}$ by adding a collision index $j$ to $\coll(i)$ for some $\pre_i \in \actt$ in line~\ref{alg2:coll2} of algorithm~\ref{infcomb}.
...
\emph{Case 4}: There is a transition $\pair_n \to \pair_{n + 1}$ by adding a vertex while processing the collision graph on line~\ref{alg2:forest} of subroutine~\ref{infcomb}.
...
Consider the construction of the collision digraph $G$ above, and let $(\forest',\coll')$ be the value of $\pair$ at line~\ref{alg2:forest}. 
Then, by (strong) induction, this pair satisfies conditions (1) and (2) of the lemma. 
...
prove a lemma about working with problem trees 
show that connection corr to containment 
