...
fix auxiliary lemma
    we don't check for true redundancy - rewrite main theorem?
    there is a dependency for this in key lemma also
        dep in case1 of key lemma - uses descendant relation
reread again to look for reductions
try to assoc problems and preallocs in a 1-1 way
strong red for active problems only to salvage?
...
**better proof for auxiliary, dealing with all diff cases (perhaps inductive for collision case, of initializing a problem at same ENDING POINT as another problem, to show that we actually do make appropriate use of information here.
    how do indices affect... information guarantees? you can pull RANK from any pair (pre,i) by looking if (pre,i) leq (pre',j) iff pre leq pre', so no problem there
