...
Similarly, an allocation $Y$ is said to be \emph{core} if there does not exist a blocking set $Z \sub X$ such that 
\[
...
\subsubsection{Extension to Core Allocations} It is tempting to attempt to extend the fixed point construction $\strcore = \fecon(T)$ and associated algorithm on the lattice $\prealloc$ to class of true core allocations $\core \supseteq \strcore$ in the general mathcing with contracts setting considered above. 
