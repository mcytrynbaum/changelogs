...
\begin{algorithmic}[1]
\REQUIRE $\acto$
    \STATE $\acta \leftarrow \acto$ \label{alg1:add1}
    \STATE Set $I = \emptyset = \fixtemp$
        ...
        \STATE $\pre \leftarrow \pre_i$
            \WHILE{$\pre_i \in \acta$} 
                ...
                \IF[Stopping conditions 2 and 3]{$\pre' = \pre$ \OR $\pre' \peq \bopre$}
                    \STATE Remove $\pre_i$ from $\acto$ and $\acta$ \label{alg1:stop1}
...
\REQUIRE $\acto$, $\coll$
    ...
    \FOR[Build Collision Digraph]{$\pre_i \in \acto$}
        \STATE Add $v_i$ to $V$
        \FOR{$j \in \coll(i)$}
            \STATE Add $(v_i,v_j)$ to $E$
        \ENDFOR
    \ENDFOR
    \STATE Set  $G = (V,E)$
    \STATE Compute the strong components $\strongc = \{\strongcomp_j\}_{j \in \mathcal{J}}$ of $G$
    \FOR[Use $G$ to Combine Information]{$j \in \mathcal{J}$}
        \STATE Compute reachable component $\reach_j$ 
        \STATE Compute tree indices $K_j = \{k \in \mathcal{I}: \exists i\in \mathcal{I} \; s.t. \; w_k = r(w_i) \: and \: v_i \in R_j\}$
        \STATE Set $\pre = \min_{k \in K_j} \pre_k$ and add to $\actt$
        \STATE Add $w(\pre)$ to $F$ with $c(w(\pre)) = \{w_i: v_i \in \strongcomp_j\}$ \label{alg2:forest}
    \ENDFOR
...
For any set $E \sub \prealloc$, we define $d(E) = \max_{\pre \in E} \|\pre\|_1$, where we identify $\prealloc$ with a grid as above\footnote{For $\x \in \mr^m$, $\|x\|_1$ is the standard 1-norm.}.
Similarly, we define $D(E) = \sum_{\pre \in E} \|\pre\|_1$.
Note that by finiteness, both $d(\cdot)$ and $D(\cdot)$ take finitely many values on subsets of $\prealloc$.
...
Note that if for any $k\geq 0$ and $\pre$ we have $\acta^{k+1} = \actak \setminus \{\pre\}$ , then $d(\acta^{k+1}) \leq d(\actak)$.
By inspection, all changes to $\acta$ except for the change on line~\ref{alg1:addq} are of this form
For line~\ref{alg1:addq}, any $\pre \in \prealloc$ has $\|\pre\|_1 > \max_i \|\pre - e_i\|$, so, in fact, if $\actak \to \acta^{k+1}$ on ~\ref{alg1:addq} then $(\acta^{k+1}) \leq d(\actak)$. 
This shows that $d(\actak)$ is a monotonically decreasing sequence. 
Since $d(\actak)$ takes at most finitely values, this sequence of real numbers eventually fixes.
...
Then $\pre \in \acta^{0}$, so, for at least one iteration of the loop~\ref{alg1:for2}, $d(\actak)$ decreases strictly.
Since $d(\actak)$ eventually fixes, the for loop at line~\ref{alg1:for2} is executed at most finitely many times. 
Then the while loop at line~\ref{alg1:while1} is executed at most finitely many times, so this subroutine terminates.

Since all inputs are finite, clearly subroutine~\ref{infcomb} terminates in finite time.

Next, we show that the iterative information combination in stage 2 terminates; that is, that the while loop at line~\ref{alg3:while2} terminates.
...
\subsubsection{Notes}
in case 1, there are points that crash
in case 2, everything bottoms out, e.g., and nothing crashes BUT the maximal point must have been remove, so it still goes down 
...
A note on throwing things away - it might be the case that we can throw away subproblems and just... leave the link in 
ugh these sub sub problems are actually really annoying - why do we keep them
note that now, single strands will be removed for lowest problem - hard to see exactly what that is until graph is fully resolved
need to add more vertices to the graph 
maybe prove some efficiency result that nothing is ever added twice

AGENDA
let acto grow 
let qc be defined 
fix vertices on collision graph - add new roots 
correctness is maintained - crucial 
automatically, we get termination for full 
just prove that while loop terminates 
    look at internals of collision dynamics
this strat brings out collisions much more
