        ...
        \STATE $\pre \leftarrow \pre_i$
            \WHILE{$\pre_i \in \acta$} 
                ...
                \IF[Stopping conditions 1 and 3]{$\pre' \peq \bopre$ \OR $\pre' = \pre$}
                    ...
                    \ELSIF{$\pre' = \pre$}
                        \FOR{$i = 1$ to $m$}
                            \IF[Add non-redundant problems]{$\nexists \pre'' \in \acto \{\pre\}$ with $\pre - e_i = \pre''$ } \label{alg1:redundant}
...
\item If $v_i, v_j \in \strongcomp$ then $\fix(\pre_i) = \fix(\pre_j)$. \label{lemma:graph:strong}
...
\end{enumerate}
\end{lemma}
In particular, item~(\ref{lemma:graph:strong}) shows that the strong components of $G$ identify groups of equivalent problems, which can then be consolidated.
...
This shows that~(\ref{lemma:graph:edge}) above holds.
...
Then, by the correspondence between directed edges and fixed point inclusion shown above, we have $\fix(\pre_i) \sub \fix(\pre_k)$ and $\fix(\pre_k) \sub \fix(\pre_i)$, so $\fix(\pre_i) = \fix(\pre_k)$ for all $v_i, v_k \in \strongcomp$, completing the proof of~(\ref{lemma:graph:strong}) above. 

Finally, if $\pre_i \in \strongcomp$ and $\pre_k \in \reach$, then there exists a path $v_i \to v_k$ by definition, so by~(\ref{lemma:graph:reach}) above there is a chain of inclusions $\fix(\pre_i) \sub \hdots \sub \fix(\pre_k)$. 
...
We will make heavy use of the collision digraph and problem forest structure lemmas proved above.

\begin{thm}
FindStrictCore finds all fixed points $\fix(T)$. 
\end{thm}
\prf
We will show that at any point during FindStrictCore, $\pre \in \fix(T)$ has either (i) $\pre \in \fixfind$ or (ii) there is a $\pre' \in \acto$ such that $\pre' \peq \pre$. 
We work by induction on the number of distinct states $\actok$ of the set of active problems $\acto$ returned by InformationAcquire and InformationCombine during the main loop of FindStrictCore\footnote{Note that $\fixfind$ changes only if $\acto$ changes, so it suffices to consider states of $\acto$} (line~\ref{alg3:while1}). 
For a set of problems $E$, we denote $\fix(E) \equiv \bigcup_{\pre \in E} \fix(\pre)$.
...
Throughout, for a set $E$, we let $E'$ denote the state of the set at algorithm initialization, and $E''$ denote the state at algorithm return.

\emph{Case 1}: There is a state change $\actok \to \acto^{k +1}$ when InformationAcquire (subroutine 1) returns.
In the above notation, $\acto^{k+1} = \actc''$ is the set of problems and $\fixtemp''$ the set of fixed points returned by the algorithm.
...
We will show by induction that if $\pre'' \in \fix(\pre)$ for some $\pre \in \acto_{\ell}$ then either $\pre'' \in \fixtemp_{\ell}$ or there exists $\pre' \in \acta_{\ell} \cup \actc_{\ell}$ with $\pre'' \peq \pre' \pre \pre$ (g).
We call such a relationship a guarantee (g) afforded by the problem $\pre' \in \acta_{\ell} \cup \actc_{\ell}$.
In particular, this will show that $\fix(\acto_{\ell}) \sub \fix(\acta_{\ell}) \cup \fix(\actc_{\ell}) \cup \fixtemp_{\ell}$ for al $\ell$.
...
But by lemma xxxx and because $\fix(T) \sub \fix(T^2) \succ \bopre$ by the check at initialization, we actually have $\fix(\pre') = \emptyset$, which is a contradiction, so $\pre \not \peq \pre_i$.
...
\emph{Case 2}: There is a state change $\actok \to \acto^{k +1}$ when InformationCombine returns.
...
We rely strongly on the collision digraph and problem forest structure results developed above.
Fix $\pre_i \in \acto'$.
By construction, we have $v_i \in G$, where $G = (V,E)$ is the collision digraph built during the algorithm.
Then $v_i \in \strongcomp_j$, a strong component of $G$ left over after we trim irrelevant singletons (line~\ref{alg2:cstrong}).
By our key lemma~\ref{lemma:forest} on the structure of the rooted forest $\forest$, we have that $\forest$ is a problem forest at any time during the execution of FindStrictCore.
Moreover, condition~(\ref{convlemma:coll}) of this lemma also holds, showing that collision indices are well-defined.
Then proposition~\ref{lemma:graph}, our result showing that fixed point relationships are reflected in the structure of the collisiion digraph also holds.
Let $\reach_j$ denote the reachable set of strong component $\strongcomp_j$.
Then by item~(\ref{lemma:graph:reach}) of this proposition, if $\pre_k \in \reach_j$, then $\fix(\pre_i) \peq \pre_k$.
With, $K_j$ the set of indices associated with this reachable set, we have $\fix(\pre_i) \peq \min_{k \in K_j} \pre_k \equiv \pre'$, which is added to $\actt$.
In particular, $\fix(\pre_i) \sub \fix(\pre')$, where $\pre' \in \actt'' = \actokk$.
Then we have shown that $\fix(\actok) \sub \fix(\actokk)$, so, in particular, $\fix(\actok) \cup \fixfind_k \sub \fix(\actokk) \cup \fixfind_{k + 1}$ since $\fixfind_k = \fixfind_{k + 1}$.

We have exhausted both cases, so this completes the induction. 
By our termination result lemma~\ref{lemma:termination}, $\acto = \emptyset$ when the algorithm returns, so we have shown that $\fix(T) \sub \fix(\acto) \cup \fixfind = \fixfind$.
Moreover, we only add $\pre$ to $\fixfind$ after checking that $T\pre = \pre$, so clearly $\fixfind \sub \fix(T)$.
Then $\fixfind = \fix(T)$. 
That is, by lemma xxxx above, the algorithm finds all strict core allocations.
\eprf
