...
First, we give a definition and an auxillary lemma. 
...
\emph{Case1}: During subroutine 1, a stopping condition is triggered and $w(\pre_i^f)$ is added to $\forest$ at line~\ref{alg1:tree1}. 
...
Then, if $\forest$ is being modified at lines~\ref{alg1:tree1} or \ref{alg1:tree2} after a stopping condition is triggered for $\pre \in \acto$, we must have $w(\pre) \in \acto$ 
We add $w(\pre_i^f)$ to $\forest$ with $c(\pre_i^f) = \pre_i \in \roott(F)$.
This modification satisfies condition~\ref{forest:child} of lemma~\ref{lemma:forest} above, so $F_{n+1}$ is still a rooted forest by the lemma above. 
...
\emph{Case 2}: During subroutine 1, a stopping condition is triggered during the processing of $\pre_i$, and $w(\pre_i^f)$ is added to $\forest$ at line~\ref{alg1:tree2}. 
